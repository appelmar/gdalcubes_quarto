...
to see what kind of expressions you can execute. Pixel band values can be accessed by name. Predefined variables that can be used within the expression include integer pixel indexes (`ix`, `iy`, `it`), and 
pixel coordinates (`left`, `right`, `top`, `bottom`), `t0`, `t1`), where the last two values are provided seconds since epoch time.
...
automatically load packages, set `load_env` and/or `load_pkgs` to `TRUE`.
