...
The new extent can be specified by spatial coordinates and datetime values (using the `extent` argument), or as zero-based integer indexes (using the `iextent` argument).
In the former case, `extent` expects a list with numeric items left, right, top, bottom, t0, and t1, or a subset thereof. In the latter case,
`iextent` is expected as a list with length-two integer vectors x, y, and t as items, defining the lower and upper cell indexes per dimension.

Notice that it is possible to crop only selected boundaries (e.g., only the right boundary) as missing boundaries in the `extent` or NA / NULL values in the `iextent` arguments are considered as "no change".
It is, however, not possible to mix arguments `extent` and `iextent`.
