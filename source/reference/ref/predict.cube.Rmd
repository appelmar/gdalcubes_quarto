...
`tidymodels`, `caret`, and simple models as from `lm` or `glm`. 

For multiple output variables and/or output in list or data.frame form, `output_names` must be provided and match 
names of the columns / items of the result object returned from the underlying predict method. For example, 
predictions using `tidymodels` return a tibble (data.frame) with columns like `.pred_class` (lassification case).
This must be explicitly provided as `output_names`. Similarly, `predict.lm` and the like produce lists
